...
"%PYTHON_EXE%" -m agent --check-admin
if %errorlevel% neq 0 (
...
:: ---------------------------------------------------------------------------
:: STEP 5 — Build the task action command
::
:: The agent is launched as:
::   cmd /c "set INSIEDR_SCHEDULED_TASK=1 && <env_loader> python -m agent"
::
:: INSIEDR_SCHEDULED_TASK=1 tells agent.py to skip the UAC re-launch guard
:: so the scheduled task never spawns a child process or shows UAC dialogs.
:: ---------------------------------------------------------------------------

:: Base command: set the scheduler env var then run the agent module
set "AGENT_CMD=set INSIEDR_SCHEDULED_TASK=1"

:: If an .env file exists, source it first via a small PowerShell one-liner
if exist "%ENV_FILE%" (
    set "AGENT_CMD=!AGENT_CMD! ^&^& for /f \"tokens=1,2 delims==\" %%A in (%ENV_FILE%) do set %%A=%%B"
    echo [InsiEDR Installer] .env file found — will be loaded at task runtime: %ENV_FILE%
) else (
    echo [InsiEDR Installer] No .env file found at %ENV_FILE%. Env vars must be set
    echo                     via System Environment Variables or Group Policy.
)

set "AGENT_CMD=!AGENT_CMD! ^&^& cd /d \"%PROJECT_ROOT%\" ^&^& \"%PYTHON_EXE%\" -m agent"

:: The full shell command passed to schtasks
set "TASK_CMD=cmd.exe /c \"!AGENT_CMD!\""
...
echo     "%PYTHON_EXE%" -m agent --status
echo.
