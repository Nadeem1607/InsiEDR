...
class LatestInferenceEngine(InferenceEngine):
    ...
    def _load_models(self):
        super()._load_models()
        # Also load target_scaler.pkl to compute scaled MSE correctly
    ...
    def predict_behavioral_risk(self, payload):
        ...
        for day in target_sequence:
            ...
            # Fused risk = max probability of any threat class
            scenario_risk = max(
        ...
        # Scale inputs using MinMax scaler
        X_flat_scaled = self.feature_scaler.transform(X_seq.reshape(-1, 1)).reshape(X_seq.shape)
        
        # Scale target using target_scaler
        y_actual_scaled = self.target_scaler.transform(y_actual.reshape(-1, 1))
        
        X_tensor = torch.tensor(X_flat_scaled, dtype=torch.float32).unsqueeze(0)
        
        # Ridge predictions (outputs scaled target prediction)
        y_pred_scaled = self.rvfl_orchestrator.predict(X_tensor, self.ridge_models)
        
        # Error is computed in SCALED space to match the orchestrator/threshold logic
        error = float(np.mean(np.square(y_actual_scaled - y_pred_scaled)))
        
        # Invert scale for behavioral risk representation
...
def analyze_user(user_id, engine, features_df, labels_df, top_users_df, test_users_list):
    ...
    for i in range(seq_len, len(daily_sequence)):
        ...
        errors.append(res["prediction_error"])
        
    max_error = np.max(errors) if errors else 0.0
    mean_error = np.mean(errors) if errors else 0.0
    anomaly_days = len(errors)
    
    # score_v2 matches the orchestrator fit
    score_v2 = max_error * mean_error * np.sqrt(anomaly_days)
