...
def test_decoy_monitor_creates_missing_file_and_baselines(collector, decoy_path):
    ...
    assert result.payload["alert_triggered"] is False
...
def test_decoy_monitor_detects_modification(collector, decoy_path):
    ...
    assert result.payload["alerts"][0]["event"] == "modified_or_accessed"
    assert result.quality == "exact"
...
def test_decoy_monitor_detects_deletion(collector, decoy_path):
    ...
    assert result.payload["alert_triggered"] is True
    assert len(result.payload["alerts"]) == 1
    assert result.payload["alerts"][0]["event"] == "deleted_and_recreated"
