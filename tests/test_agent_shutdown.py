...
def test_tamper_timeout_enforcement(agent):
    ...
    with patch("agent.agent.build_payload") as mock_build:
        ...
        # Track the timeout value during the call
        timeout_values = []
        
        original_send = agent.transport.send_or_queue
        def mock_send(*args, **kwargs):
            timeout_values.append(agent.transport.timeout_seconds)
            return original_send(*args, **kwargs)
        
        agent.transport.send_or_queue.side_effect = mock_send
        
        agent._fire_tamper_flag()
        
        # Should have recorded the 3.0 timeout during execution
        assert 3.0 in timeout_values
